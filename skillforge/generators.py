...
HEADER = "Generated by SkillForge. Review before committing."


def generate_all(root: Path, detections: List[Detection], selected_stack_ids: Iterable[str]) -> List[Path]:
    selected = [d.stack for d in detections if d.id in set(selected_stack_ids)]
    if not selected:
        return []
    outputs = {
